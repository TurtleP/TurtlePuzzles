...
for i, v in pairs(gamegui) do
	...
	for i, v in pairs(objects["turtle"]) do
		if v.x > love.graphics.getWidth()*(3/4) and #map > 25  then
			gamexscroll = v.x - love.graphics.getWidth()*(3/4)
		end
	end
...
function game_draw_HUD()
	...
	if hasedited or editormode = true then	
		love.graphics.setColor(255, 0, 0)
		love.graphics.print("Press 'E' to return to the editor!", screen_width/2.15, screen_height/4)
	end
end
...
function nextlevel()
	if not editormode or not hasedited then
